...
%description
Hime is an input method for Traditional Chinese with a GTK user interface.
...
%package qt3
Summary: Qt3 support for hime
Group:   Applications/System
Requires: qt3
Requires: %{name} = %{version}-%{release}

%description qt3
qt3 support for hime.

%package qt4
Summary: Qt4 support for hime
Group:   Applications/System
Requires: qt4
Requires: %{name} = %{version}-%{release}

%description qt4
qt4 support for hime.

%package qt5
Summary: Qt5 support for hime
Group:   Applications/System
Requires: qt5
Requires: %{name} = %{version}-%{release}
...
#sed -i -e 's|QT=qt3|QT=qt-3.3|' -e 's|/usr/lib|%{_libdir}|' qt-im/Makefile
sed -i 's|-lm|-lm -Wl,--allow-multiple-definition|' configure

%build
%configure --with-gtk=2.0 --disable-qt6-immodule
make
...
# devel files {
rm -f $RPM_BUILD_ROOT/%{_includedir}/hime-im-client.h
rm -f $RPM_BUILD_ROOT/%{_libdir}/libhime-im-client.so
# }
...
#files qt3
#{_libdir}/qt-3.3/plugins/inputmethods/im-hime.so

#files qt4
#{_libdir}/qt4/plugins/inputmethods/im-hime.so

%files qt5
%{_libdir}/qt5/plugins/platforminputcontexts/im-hime.so

%changelog
* Sun Jul 10 2022 Wei-Lun Chao <bluebat@member.fsf.org> - 0.10.1
- Rebuilt for Fedora
* Sat Dec 10 2011 caleb - 0.9~rc1
