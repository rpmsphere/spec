...
BuildRequires:  python2-devel numpy atlas-devel python2-setuptools
...
find $RPM_BUILD_ROOT%{python_sitearch}/pysparse -type f -name *.py -exec %__chmod 755 {} \;
%__chmod 755 $RPM_BUILD_ROOT%{python_sitearch}/pysparse/sparse/*sparseMatrix.py 
...
find $RPM_BUILD_ROOT%{python_sitearch} -type f -name *.pyc -exec %__sed -i 's|$RPM_BUILD_ROOT||g' {} \;

sed -i 's|/usr/bin/env python$|/usr/bin/python2|' %{buildroot}%{python_sitearch}/pysparse/*.py %{buildroot}%{python_sitearch}/pysparse/*/*.py
 
%clean
rm -rf $RPM_BUILD_ROOT

%files
%doc CHANGES LICENSE README TODO examples test doc/pysparse/build/html
%{python_sitearch}/*
