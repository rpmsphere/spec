...
BuildArch:      noarch
BuildRequires:  python2-devel, fdupes, python2-setuptools
...
chmod +x $RPM_BUILD_ROOT/%{python_sitelib}/%{pyname}_lib/pad/{pad.py,setup_pad.py}
chmod +x $RPM_BUILD_ROOT/%{python_sitelib}/%{pyname}_lib/{sparser.py,filter.py}
chmod +x $RPM_BUILD_ROOT/%{python_sitelib}/%{pyname}_lib/pyparsing/setup.py

sed -i 's|/usr/bin/env python$|/usr/bin/python2|' %{buildroot}%{python_sitelib}/tappy_lib/*.py %{buildroot}%{python_sitelib}/tappy_lib/*/*.py
