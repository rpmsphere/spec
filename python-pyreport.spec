...
Version:        0.3.4c
Release:        22.1
Summary:        Pyreport makes notes out of a python script
License:        BSD
...
CFLAGS="%{optflags} " %{__python} setup.py bdist_egg
...
find $RPM_BUILD_ROOT/%{python_sitelib} -name "*.pyc" -exec rm -f '{}' \;

# fix some exec bits
#find $RPM_BUILD_ROOT/%{python_sitelib} -name "*.py" -exec chmod -x '{}' \;
#chmod +x $RPM_BUILD_ROOT/%{python_sitelib}/%{pyname}-%{version}-py*.egg/%{pyname}/pyreport.py
