...
Name:      turbovnc
Version:   2.1
Release:   20160920%{?dist}
URL:       http://www.TurboVNC.org
Source0: http://prdownloads.sourceforge.net/turbovnc/%{name}-%{version}.tar.gz
License:   GPL
Group:     User Interface/Desktops
Requires:  bash >= 2.0
Requires:  /sbin/chkconfig %{_sysconfdir}/init.d
BuildRoot: %{_tmppath}/%{name}-%{version}-root
BuildRequires: perl libjpeg-turbo-devel turbojpeg-devel cmake
BuildRequires: /usr/bin/perl
BuildRequires: zlib-devel libXext-devel libX11-devel
BuildRequires: libX11-devel automake autoconf libtool libXcursor-devel
...
Buildrequires: pam-devel freetype-devel desktop-file-utils
Requires: turbojpeg VirtualGL
...
%endif
cmake -G"Unix Makefiles" -DCMAKE_INSTALL_PREFIX=%{prefix} -DTVNC_BINDIR=%{bindir} -DTVNC_CONFDIR=%{confdir} -DTVNC_DOCDIR=%{docdir} -DTVNC_MANDIR=%{mandir} -DTVNC_BUILDJAVA=%{java} $JNIJARS_ARGS -DCMAKE_BUILD_TYPE=RelWithDebInfo -DCMAKE_VERBOSE_MAKEFILE=FALSE $JAVA_KEYSTORE_ARG $JAVA_KEYSTORE_PASS_ARG $JAVA_KEYSTORE_TYPE_ARG $JAVA_KEY_ALIAS_ARG $JAVA_KEY_PASS_ARG $JAVA_TSA_URL_ARG -DTJPEG_INCLUDE_DIR=%{_includedir} -DTJPEG_LIBRARY=%{_libdir}/libturbojpeg.so .
make
...
mkdir -p $RPM_BUILD_ROOT/opt/share/Modules/modulefiles/turbovnc
cat > $RPM_BUILD_ROOT/opt/share/Modules/modulefiles/turbovnc/%{version} <<ENDMODULE
#%Module

proc ModulesHelp { } {
   puts stderr "This module adds turbovnc to various paths"
}

module-whatis   "Sets up turbovnc in your environment"
...
%post
if [ "$1" = 1 ]; then
  if [ -e /etc/redhat-release ]; then /sbin/chkconfig --add tvncserver; fi
fi
...
if [ "$1" = 0 ]; then
  ...
  if [ -e /etc/redhat-release -a -f /etc/init.d/tvncserver ]; then
...
%files
%defattr(-,root,root)
%dir /opt/share/Modules/modulefiles/turbovnc
/opt/share/Modules/modulefiles/turbovnc/%{version}
%if "%{server}" == "1"
 ...
 %config(noreplace) %{confdir}/turbovncserver-security.conf
%endif
